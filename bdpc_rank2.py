...
"""
查首页自然排名(即含快照)前10名
关键词文件kwd.txt,一行一个,指定待查询domain
domain不要带https或者http
结果保存：关键词 对应排名的url 及排名值
某域名下一个词有2个url排名,取第一个
...
class BdpcRank2(threading.Thread):
                ...
                if encrypt_url_list_rank:
                    ...
                    for real_url in url_keys:
                        if domain in real_url:
                            rank = rank_dict[real_url]
                            f.write(kwd+'\t'+real_url+'\t'+str(rank)+'\n')
                            print(kwd,real_url,rank)                        
